...
def test_pattern_p_invoke_with_stub_provider():
    ...
    with patch("agent_cost_bench.eie.pattern_paper.call_llm", side_effect=lambda **kw: fake_call_llm(**kw)), \
         patch("agent_cost_bench.eie.pattern_paper.dispatch_tool_call", side_effect=fake_dispatch):
        handler = StatusOnlyHandler()
