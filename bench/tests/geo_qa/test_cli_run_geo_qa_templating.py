"""CLI subcommand `run-geo-qa-templating` tests.
...
def test_help_lists_subcommand():
    ...
    assert "run-geo-qa-templating" in result.output


def test_subcommand_help_exits_0():
    result = runner.invoke(app, ["run-geo-qa-templating", "--help"])
    assert result.exit_code == 0
...
def test_all_scenarios_run(tmp_path):
    ...
    with patch("agent_cost_bench.cli.run_geo_qa_scenario") as mock_run:
        mock_run.side_effect = lambda cfg, **kwargs: fake_paths[cfg.id]
        result = runner.invoke(app, ["run-geo-qa-templating", "--scenario", "all"])
...
def test_recursion_limit_threaded(tmp_path):
    ...
    with patch("agent_cost_bench.cli.run_geo_qa_scenario") as mock_run:
        ...
        result = runner.invoke(
            app,
            ["run-geo-qa-templating", "--scenario", sid, "--recursion-limit", "60"],
        )
...
def test_recursion_limit_defaults_to_30(tmp_path):
    ...
    with patch("agent_cost_bench.cli.run_geo_qa_scenario") as mock_run:
        mock_run.return_value = fake_path
        result = runner.invoke(app, ["run-geo-qa-templating", "--scenario", sid])
...
def test_model_override(tmp_path):
    ...
    with patch("agent_cost_bench.cli.run_geo_qa_scenario") as mock_run:
        ...
        result = runner.invoke(
            app,
            ["run-geo-qa-templating", "--scenario", sid, "--model", "my-test-model"],
        )
...
def test_output_printed(tmp_path):
    ...
    with patch("agent_cost_bench.cli.run_geo_qa_scenario", return_value=fake_path):
        result = runner.invoke(app, ["run-geo-qa-templating", "--scenario", sid])
...
def test_run_scenario_exception_propagates(tmp_path):
    ...
    with patch(
        "agent_cost_bench.cli.run_geo_qa_scenario",
