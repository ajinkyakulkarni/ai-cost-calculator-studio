...
def test_cli_preview_uses_aoi_bbox_not_item_bbox(monkeypatch) -> None:
    ...
    from agent_cost_bench.geo_qa.schemas import GeocodeReturn, SearchItemsReturn, StacItemFields
    ...
    from agent_cost_bench.geo_qa import map_preview
    monkeypatch.setattr(map_preview, "render_preview", _fake_render)

    result = CliRunner().invoke(
        cli.app,
        ["preview-geo-qa-templating", "--max-items", "1"],
    )
