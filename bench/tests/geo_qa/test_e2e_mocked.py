...
Patches:
  - provider_shim.call_llm  (via pattern_paper / pattern_gated) → hard-coded sequence
...
def test_e2e_paper_status_only(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "agent_cost_bench.geo_qa.pattern_paper.call_llm",
        lambda **kw: next(seq),
    )
    monkeypatch.setattr(
        "agent_cost_bench.geo_qa.pattern_paper.dispatch_tool_call",
        _stub_dispatch,
    )

    cfg = load_scenario(SCENARIO_DIR / "pattern-paper-status-only.yml")

    with patch("agent_cost_bench.geo_qa.runner.REPORTS_DIR", tmp_path):
...
def test_e2e_all_scenarios_produce_valid_trace(scenario_id, tmp_path):
    ...
    is_gated = cfg.pattern == "gated"

    seq = _gated_llm_sequence() if is_gated else _paper_llm_sequence()
    call_llm_module = (
        "agent_cost_bench.geo_qa.pattern_gated.call_llm"
        if is_gated
        else "agent_cost_bench.geo_qa.pattern_paper.call_llm"
    )
    dispatch_module = (
        "agent_cost_bench.geo_qa.pattern_gated.dispatch_tool_call"
        if is_gated
        else "agent_cost_bench.geo_qa.pattern_paper.dispatch_tool_call"
    )
...
def test_e2e_report_shape_from_six_traces(tmp_path):
    ...
    _scenarios_meta = [
        ...
        ("pattern-gated-status-only",   "gated",   "status_only"),
        ("pattern-gated-key-fields",    "gated",   "key_fields"),
        ("pattern-gated-freeform",      "gated",   "freeform"),
    ]
