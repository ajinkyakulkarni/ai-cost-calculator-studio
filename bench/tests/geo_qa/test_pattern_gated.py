"""Pattern Gated — 5-gate drill-down state machine.
...
def test_pattern_e_invoke_with_stub():
    ...
    with patch("agent_cost_bench.geo_qa.pattern_gated.call_llm", side_effect=lambda **kw: fake_llm(**kw)), \
         patch("agent_cost_bench.geo_qa.pattern_gated.dispatch_tool_call", side_effect=fake_dispatch):
        handler = KeyFieldsHandler()
        actor = UserActor.frozen_default()
