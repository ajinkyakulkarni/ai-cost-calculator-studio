"""CLI subcommand `report-geo-qa-templating` tests.
...
def test_help_lists_report_subcommand():
    ...
    assert "report-geo-qa-templating" in result.output


def test_report_subcommand_help_exits_0():
    result = runner.invoke(app, ["report-geo-qa-templating", "--help"])
    assert result.exit_code == 0
...
def test_report_prints_output_path(tmp_path):
    ...
    with patch("agent_cost_bench.geo_qa.report.emit_report", return_value=fake_out):
        result = runner.invoke(app, ["report-geo-qa-templating"])
...
def test_report_exception_propagates():
    with patch(
        "agent_cost_bench.geo_qa.report.emit_report",
