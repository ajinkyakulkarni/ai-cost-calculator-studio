...
STAC calls happen. We verify:
  ...
  4. Pattern 'paper' and pattern 'gated' both route to the right graph.
...
def test_run_scenario_gated_returns_trace_path(tmp_path):
    cfg = ScenarioCfg(
    ...
    responses = iter(_gated_stub_responses())

    with patch("agent_cost_bench.geo_qa.pattern_gated.call_llm",
               side_effect=lambda **kw: next(responses)), \
         patch("agent_cost_bench.geo_qa.pattern_gated.dispatch_tool_call",
               side_effect=_fake_dispatch), \
         patch("agent_cost_bench.geo_qa.runner.REPORTS_DIR", tmp_path):
        out_path = run_scenario(cfg)

    assert out_path.exists()
    assert "pattern-gated-status-only" in out_path.name


def test_run_scenario_gated_turn_count(tmp_path):
    cfg = ScenarioCfg(
...
def test_enforce_compute_stats_gated_injects_instruction_into_system_prompt(tmp_path):
    """When cfg.enforce_compute_stats=True, system prompt passed to call_llm contains
    the forced instruction for an gated-pattern scenario."""
    cfg = ScenarioCfg(
        id="pattern-gated-status-only",
        pattern="gated",
        handler_mode="status_only",
